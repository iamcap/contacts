...
function setupConsoleTest() {
  ...
  log4js = sandbox.require('../lib/log4js', {globals: {console: fakeConsole}});
...
vows.describe('log4js').addBatch({
    ...
    topic: function() {
      ...
      var events = {
        ...
        shutdownCallbackCalled: false
          ...
          log4js = sandbox.require('../lib/log4js', {requires: {'./appenders/file': {
                name: "file",
                appender: function() {},
  ...
  'invalid configuration': {'should throw an exception': function() {
  ...
  'configuration that causes an error': {
    topic: function() {
      var log4js = sandbox.require('../lib/log4js', {requires: {'./appenders/file': {
            name: "file",
            appender: function() {},
            configure: function(configuration) {
  ...
  'configuration when passed as filename': {
    topic: function() {
      var appenderConfig,
          configFilename,
          log4js = sandbox.require('../lib/log4js', {requires: {
              'fs': {
                statSync: function() {
                ...
                readFileSync: function(filename) {
                  ...
                  return JSON.stringify({appenders: [{
                      type: "file",
  ...
  'with no appenders defined': {
    topic: function() {
      var logger,
          ...
          log4js = sandbox.require('../lib/log4js', {requires: {'./appenders/console': fakeConsoleAppender}});
  ...
  'addAppender': {
    ...
    'without a category': {
      ...
      'if an appender for a category is defined': {'should register for that category': function(log4js) {
          var otherEvent,
              appenderEvent,
              cheeseLogger;
          log4js.addAppender(function(evt) {
    ...
    'with a category': {'should only register the function as a listener for that category': function(log4js) {
        var appenderEvent,
            appender = function(evt) {
              appenderEvent = evt;
            },
  ...
  'default setup': {
    topic: function() {
      var appenderEvents = [],
          ...
          globalConsole = {log: function() {}},
          log4js = sandbox.require('../lib/log4js', {
            requires: {'./appenders/console': fakeConsole},
            globals: {console: globalConsole}
          }),
