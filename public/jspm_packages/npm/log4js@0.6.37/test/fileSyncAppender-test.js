...
vows.describe('log4js fileSyncAppender').addBatch({
  ...
  'with a max file size and no backups': {
    topic: function() {
      ...
      remove(testFile + '.1');
      log4js.clearAppenders();
      log4js.addAppender(require('../lib/appenders/fileSync').appender(testFile, log4js.layouts.basicLayout, 100, 0), 'max-file-size');
  ...
  'with a max file size and 2 backups': {
    topic: function() {
      ...
      remove(testFile + '.2');
      log4js.clearAppenders();
      log4js.addAppender(require('../lib/appenders/fileSync').appender(testFile, log4js.layouts.basicLayout, 50, 2), 'max-file-size-backups');
      logger.info("This is the first log message.");
      logger.info("This is the second log message.");
    ...
    'the log files': {
      ...
      'should be named in sequence': function(files) {
        assert.deepEqual(files, ['fa-maxFileSize-with-backups-sync-test.log', 'fa-maxFileSize-with-backups-sync-test.log.1', 'fa-maxFileSize-with-backups-sync-test.log.2']);
      },
...
}).addBatch({'configure': {'with fileSyncAppender': {
      topic: function() {
