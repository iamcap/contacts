...
function setupLogging(category, options) {
  ...
  var fakeHipchatNotifier = {'make': function(room, token, from, host, notify) {
      return {
        'room': room,
        'token': token,
        'from': from || '',
  ...
  var hipchatModule = sandbox.require('../lib/appenders/hipchat', {requires: {'hipchat-notifier': fakeHipchatNotifier}});
...
vows.describe('HipChat appender').addBatch({
  'when logging to HipChat v2 API': {
    topic: function() {
      var customCallback = function(err, res, body) {
        return 'works';
      };
  ...
  'when missing options': {
    topic: function() {
      var setup = setupLogging('myLogger', {"type": "hipchat"});
