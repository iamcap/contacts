...
(function(process) {
  ...
  DateRollingFileStream = require('../../lib/streams/index').DateRollingFileStream;
  ...
  vows.describe('DateRollingFileStream').addBatch({
    ...
    'with stream arguments': {
      topic: new DateRollingFileStream(__dirname + '/test-date-rolling-file-stream-3', 'yyyy-MM-dd', {mode: parseInt('0666', 8)}),
    ...
    'with stream arguments but no pattern': {
      topic: new DateRollingFileStream(__dirname + '/test-date-rolling-file-stream-4', {mode: parseInt('0666', 8)}),
