...
(function(process) {
  ...
  RollingFileStream = require('../../lib/streams/index').RollingFileStream;
  ...
  vows.describe('RollingFileStream').addBatch({
    ...
    'with stream arguments': {
      topic: function() {
        remove(__dirname + '/test-rolling-file-stream');
        return new RollingFileStream('test-rolling-file-stream', 1024, 5, {mode: parseInt('0666', 8)});
      },
    ...
    'writing less than the file size': {
      topic: function() {
        ...
        var that = this,
            stream = new RollingFileStream(__dirname + "/test-rolling-file-stream-write-less", 100);
      ...
      'the number of files': {
        ...
        'should be one': function(files) {
          assert.equal(files.filter(function(file) {
    ...
    'writing more than the file size': {
      topic: function() {
        ...
        var that = this,
            stream = new RollingFileStream(__dirname + "/test-rolling-file-stream-write-more", 45);
    ...
    'when many files already exist': {
      topic: function() {
        ...
        create(__dirname + '/test-rolling-stream-with-existing-files.1');
        var that = this,
            stream = new RollingFileStream(__dirname + "/test-rolling-stream-with-existing-files", 45, 5);
        write7Cheese(that, stream);
      },
      'the files': {
        topic: function() {
