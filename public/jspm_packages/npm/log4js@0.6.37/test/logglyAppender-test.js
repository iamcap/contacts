...
function setupLogging(category, options) {
  var msgs = [];
  var fakeLoggly = {createClient: function(options) {
      return {
        config: options,
...
vows.describe('log4js logglyAppender').addBatch({'with minimal config': {
    topic: function() {
  ...
  }}).addBatch({'config with object with tags and other keys': {
    topic: function() {
      var setup = setupTaggedLogging();
    ...
    'has a result msg with the args concatenated': function(topic) {
      assert.equal(topic.results[0].msg.msg, 'Log event #1 { other: \'other\', tags: [ \'tag1\', \'tag2\' ] }');
    },
