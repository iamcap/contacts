...
(function(process) {
  ...
  vows.describe('log4js configure').addBatch({'appenders': {
      'when specified by type': {
        topic: function() {
          var testAppender = makeTestAppender(),
              log4js = sandbox.require('../lib/log4js', {requires: {'./appenders/cheese': testAppender}});
      ...
      'when appender in node_modules loaded via loadAppender': {
        topic: function() {
          var testAppender = makeTestAppender(),
              log4js = sandbox.require('../lib/log4js', {requires: {'some/other/external': testAppender}});
      ...
      'when configuration file loaded via LOG4JS_CONFIG environment variable': {
        topic: function() {
          ...
          var fileRead = 0,
              ...
              log4js = sandbox.require('../lib/log4js', {requires: {'fs': fakeFS}});
