      ...
      setupLogging = function(options, category, compressedLength) {
        var fakeDgram = {
          ...
          createSocket: function(type) {
            ...
            fakeZlib = {gzip: function(objectToCompress, callback) {
            ...
            exitHandler,
            fakeConsole = {error: function(message) {
            ...
            appender = sandbox.require('../lib/appenders/gelf', {
              ...
              globals: {
                process: {on: function(evt, handler) {
                    if (evt === 'exit') {
                      exitHandler = handler;
                    }
                  }},
  ...
  vows.describe('log4js gelfAppender').addBatch({
    'with default gelfAppender settings': {
      ...
      'the uncompressed log message': {
        ...
        'should be in the gelf format': function(message) {
          ...
          assert.equal(message.host, require('@empty').hostname());
          assert.equal(message.level, 6);
    ...
    'with custom fields options': {
      ...
      'the uncompressed packet': {
        ...
        'should pick up the options': function(message) {
          assert.equal(message.host, 'cheese');
          assert.isUndefined(message.GELF);
          assert.equal(message._facility, 'nonsense');
          assert.equal(message._every1, 'Hello every one');
          assert.equal(message._every2, 'Overwritten!');
          assert.equal(message._myField, 'This is my field!');
          assert.equal(message.short_message, 'Just testing.');
        }
      }
    }
  }).export(module);
