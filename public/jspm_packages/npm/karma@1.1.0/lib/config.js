...
(function(process) {
  ...
  var createPatternObject = function(pattern) {
    ...
    if (helper.isObject(pattern)) {
      if (pattern.pattern && helper.isString(pattern.pattern)) {
        return helper.isUrlAbsolute(pattern.pattern) ? new UrlPattern(pattern.pattern) : new Pattern(pattern.pattern, pattern.served, pattern.included, pattern.watched, pattern.nocache);
      }
  ...
  var normalizeConfig = function(config, configFilePath) {
    ...
    if (config.protocol.match(/https?:/) === null) {
      log.warn('"%s" is not a supported protocol, defaulting to "http:"', config.protocol);
      config.protocol = 'http:';
    }
    if (config.proxies && config.proxies.hasOwnProperty(config.urlRoot)) {
      log.warn('"%s" is proxied, you should probably change urlRoot to avoid conflicts', config.urlRoot);
    }
    ...
    types.forEach(function(type) {
      ...
      Object.keys(definitions).forEach(function(name) {
        ...
        var locals = {args: ['value', definition]};
