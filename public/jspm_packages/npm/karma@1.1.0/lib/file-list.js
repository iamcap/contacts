...
List.prototype._findFile = function(path, pattern) {
  if (!path || !pattern)
    return;
  if (!this.buckets.has(pattern.pattern))
    return;
...
Object.defineProperty(List.prototype, 'files', {get: function() {
    var self = this;
...
List.prototype.addFile = function(path) {
  ...
  return Promise.all([fs.statAsync(path), this._refreshing]).spread(function(stat) {
...
List.$inject = ['config.files', 'config.exclude', 'emitter', 'preprocess', 'config.autoWatchBatchDelay'];
module.exports = List;
