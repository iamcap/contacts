...
var baseDirFromPattern = function(pattern) {
  return pattern.replace(/[\/\\][^\/\\]*\*.*$/, '').replace(/[\/\\][^\/\\]*[!\+]\(.*$/, '').replace(/[\/\\][^\/\\]*\)\?.*$/, '') || DIR_SEP;
};
...
exports.watch = function(patterns, excludes, fileList, usePolling, emitter) {
  ...
  chokidarWatcher.on('add', bind(fileList.addFile)).on('change', bind(fileList.changeFile)).on('unlink', bind(fileList.removeFile)).on('error', function(e) {
    log.debug(e);
  });
...
exports.watch.$inject = ['config.files', 'config.exclude', 'fileList', 'config.usePolling', 'emitter'];
