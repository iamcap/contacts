...
var getXUACompatibleMetaElement = function(url) {
  ...
  if (urlObj.query['x-ua-compatible']) {
    tag = '\n<meta http-equiv="X-UA-Compatible" content="' + urlObj.query['x-ua-compatible'] + '"/>';
  }
  return tag;
};
...
var createKarmaMiddleware = function(filesPromise, serveStaticFile, serveFile, injector, basePath, urlRoot) {
  return function(request, response, next) {
    ...
    if (isRequestingJsFile) {
      return serveStaticFile(requestUrl, response, function(data) {
        return data.replace('%KARMA_URL_ROOT%', urlRoot).replace('%KARMA_VERSION%', VERSION);
      });
    }
    ...
    if (isRequestingContextFile || isRequestingDebugFile) {
      return filesPromise.then(function(files) {
        ...
        serveStaticFile(requestUrl, response, function(data) {
          common.setNoCacheHeaders(response);
          return data.replace('%SCRIPTS%', '').replace('%CLIENT_CONFIG%', '').replace('%MAPPINGS%', 'window.__karma__.error("TEST RUN WAS CANCELLED because ' + (errorFiles.length > 1 ? 'these files contain' : 'this file contains') + ' some errors:\\n  ' + errorFiles.join('\\n  ') + '");');
        });
      });
      ...
      return filesPromise.then(function(files) {
        ...
        response.end(JSON.stringify({files: files.included.map(function(file) {
...
createKarmaMiddleware.$inject = ['filesPromise', 'serveStaticFile', 'serveFile', 'injector', 'config.basePath', 'config.urlRoot'];
