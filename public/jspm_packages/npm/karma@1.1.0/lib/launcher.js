...
(function(process) {
  ...
  var baseBrowserDecoratorFactory = function(baseLauncherDecorator, captureTimeoutLauncherDecorator, retryLauncherDecorator, processLauncherDecorator) {
  ...
  var Launcher = function(server, emitter, injector) {
      ...
      return function(name) {
        ...
        try {
          ...
          if (e.message.indexOf('No provider for "launcher:' + name + '"') !== -1) {
            log.error('Cannot load browser "%s": it is not registered! ' + 'Perhaps you are missing some plugin?', name);
          } else {
    ...
    this.launch = function(names, concurrency) {
      log.info('Launching browser%s %s with %s', names.length > 1 ? 's' : '', names.join(', '), concurrency === Infinity ? 'unlimited concurrency' : 'concurrency ' + concurrency);
    ...
    this.launchSingle.$inject = ['config.protocol', 'config.hostname', 'config.port', 'config.urlRoot'];
    ...
    this.markCaptured = function(id) {
      self._browsers.forEach(function(browser) {
        if (browser.id === id) {
          ...
          log.debug('%s (id %s) captured in %d secs', browser.name, browser.id, (Date.now() - lastStartTime) / 1000);
        }
      });
    };
